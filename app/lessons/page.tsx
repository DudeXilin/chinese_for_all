...
export default function LessonsPage() {
  ...
  const onPointerUp = (event: PointerEvent<HTMLDivElement>) => {
    ...
    if (Math.abs(delta) >= 45) {
      // Drag right = previous section, drag left = next section.
  ...
  const onNavigatorClick = (event: PointerEvent<HTMLDivElement>) => {
    ...
    goTo(Math.round(ratio * (sections.length - 1)));
  };
  ...
  return (
    <main className="lessons-page">
        ...
        >
          <section className="lesson-panel">
            <div className="lesson-placeholder">
              <span>Здесь будут колоды с тематиками</span>
            </div>
          </section>
          <section className="lesson-panel">
            <div className="lesson-placeholder">
              <span>Здесь будут упражнения по грамматике</span>
            </div>
          </section>
          <section className="lesson-panel">
            <div className="lesson-placeholder">
              <span>Здесь будет 3 тип упражнений</span>
            </div>
          </section>
          <section className="lesson-panel">
            <div className="lesson-placeholder">
              <span>Здесь будет 4 тип упражнений</span>
            </div>
          </section>
        </div>
      </div>
      ...
      >
        <div className="lessons-navigator-track">
          {sections.map((section) => (
            <span key={section} className="lessons-navigator-stop" />
          ))}
        </div>
        <button
          type="button"
          ...
          aria-label={`Выбрано: ${sections[active]}`}
          tabIndex={-1}
        >
          {sections[active]}
        </button>
      ...
      <style jsx>{`
        ...
        .lessons-navigator-stop:nth-child(1) { left: 0%; }
        .lessons-navigator-stop:nth-child(2) { left: 33.333%; }
        .lessons-navigator-stop:nth-child(3) { left: 66.666%; }
        .lessons-navigator-stop:nth-child(4) { left: 100%; }

        .lessons-navigator-thumb {
          position: absolute;
          top: 50%;
          width: max-content;
          min-width: 42%;
